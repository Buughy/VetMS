...
export function registerProducts(server: FastifyInstance, db: Db) {
  server.get('/api/products', async (req: FastifyRequest<{ Querystring: { query?: string } }>) => {
    const query = req.query.query?.trim();
    if (query) {
      return db
        .prepare(
          'SELECT id, name, price FROM products WHERE name LIKE ? ORDER BY name LIMIT 50'
        )
        .all(`%${query}%`);
    }
    return db
  ...
  server.post('/api/products', async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post(
    ...
    async (req: FastifyRequest<{ Body: { csv?: string } }>, reply: FastifyReply) => {
      const csv = req.body.csv;
      if (!csv || typeof csv !== 'string') return reply.code(400).send({ error: 'csv required' });

      const rows = csv.split(/\r?\n/).map((r) => r.trim()).filter(Boolean);
      if (rows.length === 0) return reply.code(400).send({ error: 'No rows found' });
      ...
      if (processed === 0) return reply.code(400).send({ error: 'No valid rows found' });
