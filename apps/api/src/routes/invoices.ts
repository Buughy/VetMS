...
function nextFriendlyId(db: Db) {
  const row = db
    .prepare(
      "SELECT friendly_id FROM invoices WHERE friendly_id LIKE 'MBV-%' ORDER BY id DESC LIMIT 1"
    )
    .get() as { friendly_id?: string } | undefined;
...
export function registerInvoices(server: FastifyInstance, db: Db) {
  ...
  server.get(
    ...
    async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      const items = db
        .prepare(
          `SELECT ii.id, ii.pet_id, ii.product_id, ii.product_name_snapshot, ii.quantity, ii.price_snapshot,
  ...
  server.put(
    ...
    async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      const tx = db.transaction(() => {
        // Client upsert
        db.prepare(
          'INSERT INTO clients(name, contact_info) VALUES(?,?) ON CONFLICT(name) DO UPDATE SET contact_info=COALESCE(excluded.contact_info, clients.contact_info)'
        ).run(data.clientName.trim(), data.contactInfo?.trim() ?? null);

        const client = db
          .prepare('SELECT id FROM clients WHERE name=?')
          .get(data.clientName.trim()) as { id: number };
        ...
        db.prepare(
          'UPDATE invoices SET client_id=?, pet_id=NULL, date=?, status=?, total_amount=? WHERE id=?'
        ).run(client.id, dateStr, data.status, total, id);

        // Replace items
        db.prepare('DELETE FROM invoice_items WHERE invoice_id=?').run(id);

        const itemStmt = db.prepare(
          'INSERT INTO invoice_items(invoice_id, pet_id, product_id, product_name_snapshot, quantity, price_snapshot) VALUES(?,?,?,?,?,?)'
        );

        for (const petData of data.pets) {
          // Pet upsert
          db.prepare(
            'INSERT INTO pets(name, species, client_id) VALUES(?,?,?) ON CONFLICT(name, client_id) DO UPDATE SET species=COALESCE(excluded.species, pets.species)'
          ).run(petData.petName.trim(), petData.petSpecies?.trim() ?? null, client.id);

          const pet = db
            .prepare('SELECT id FROM pets WHERE name=? AND client_id=?')
            .get(petData.petName.trim(), client.id) as { id: number };
  ...
  server.post('/api/invoices', async (req: FastifyRequest, reply: FastifyReply) => {
    const parsed = createInvoiceSchema.safeParse(req.body);
    if (!parsed.success) return reply.code(400).send(parsed.error);
    ...
    const tx = db.transaction(() => {
      ...
      const invoiceRes = db
        .prepare(
          'INSERT INTO invoices(friendly_id, client_id, pet_id, date, status, total_amount) VALUES(?,?,NULL,?,?,?)'
        )
