...
export function registerDashboard(server, db) {
    server.get('/api/dashboard', async () => {
        ...
        const countRow = db
            ...
            .get(today);
        const topRow = db
            .prepare(`SELECT product_name_snapshot as name, SUM(quantity) as qty
    ...
    server.get('/api/stats', async (req, reply) => {
        ...
        const monthlyStats = db
            .prepare(`SELECT substr(date, 1, 7) as month,
                  COALESCE(SUM(total_amount), 0) as revenue,
                  COUNT(*) as count
           FROM invoices
           WHERE date >= ? AND date <= ?
           ...
           ORDER BY month`)
            .all(from, to);
        const topItems = db
            .prepare(`SELECT ii.product_name_snapshot as name,
