...
export function registerClients(server, db) {
    server.get('/api/clients', async (req) => {
        const query = req.query.query?.trim();
        if (query) {
            return db
                .prepare('SELECT id, name, contact_info FROM clients WHERE name LIKE ? ORDER BY name LIMIT 20')
                .all(`%${query}%`);
        }
        return db
    ...
    server.post('/api/clients', async (req, reply) => {
    ...
    server.put('/api/clients/:id', async (req, reply) => {
    ...
    server.delete('/api/clients/:id', async (req, reply) => {
        const id = Number(req.params.id);
        if (!Number.isFinite(id))
            return reply.code(400).send({ error: 'Invalid id' });
